...
(function ($) {
  ...
  $('button[name="approve"]').click(function (e) {
    ...
    $.ajax({
      url: `/api/application-approve/${uuid}`,
      type: 'get',
      ...
      success: function (response) {
        if (response.status) {
          Swal.fire(
            'Success',
            'You have approve the application.',
            'success'
          ).then(function (dialogue_result) {
            if (dialogue_result.isConfirmed) {
          ...
          Swal.fire(
            'Error',
            'Something went wrong. Please try again or contact with a administrator.',
            'error'
          );
        }
      },
      error: function (err) {
        Swal.fire(
          'Error',
          'Something went wrong. With error: ' + err,
          'error'
        );
      }
    });
  ...
  $('button[name="reject"]').click(function (e) {
    ...
    $.ajax({
      url: `/api/application-reject/${uuid}`,
      type: 'get',
      contentType: 'application/json; charset=utf-8',
      success: function (response) {
        if (response.status) {
          Swal.fire(
            'Success',
            'You have reject the application.',
            'success'
          ).then(function (dialogue_result) {
            if (dialogue_result.isConfirmed) {
              document.location.reload();
  ...
  function getCounts() {
    ...
    $.ajax({
      url: '/api/notification/getCount',
      type: 'get',
      async: false,
      contentType: 'application/json; charset=utf-8',
      success: function (response) {
        if (response.status) {
          $(".notification-unread-count").html(response.object);
        }
      },
      error: function(err){
        console.error(err);
  ...
  var notification_objs = [];
  $(".notification-view").click(function () {
    $('.notification-dropdown-body').html('');
    $.ajax({
      ...
      success: function (response) {
        if (response.status) {
          ...
          response.object.forEach(element => {
            ...
            var obj = {
              application_guid: element.uuid,
              issuer: element.sender_family_name,
              date_submitted: element.application.date_submitted,
              dates_submitted: element.application.dates_submitted,
              total_days: element.application.days,
              not_status: element.application.statusId,
              reason: element.application.reason,
              notification_action: element.notification_action,
              notification_id : element.id,
            };
            notification_objs.push(obj)

            var row = "";

            row += `<a href="#" class="dropdown-item view-more-modal-notification" data-id=${element.id}>`;
            row += `<i class="fas fa-users mr-2"></i> ${element.notification_action == 0 ? "You need to approve a new application" : "You have answer from one of your applications."}`;
            row += `<span class="float-right text-muted text-sm">View More</span>`;
            row += '<br/>'
            row += `<sub>${element.notification_inserted_date}</sub>`
            row += '</a>';
            row += '<div class="dropdown-divider"></div>';

            $('.notification-dropdown-body').append(row);
  ...
  $(document).on('click', '.view-more-modal-notification', function () {

    $('.view-more-notification').modal('show');
    var notification_obj;

    notification_objs.forEach(element => {
      if(element.notification_id == parseInt($(this).attr('data-id'))){
        notification_obj = element;
      }

    });


    $('.modal-title').html(notification_obj.notification_action == 0 ? "You need to approve a new application" : "You have answer from one of your applications");

    var text = "";
    if (notification_obj.notification_action == 0) {
      text = `The employee ${notification_obj.issuer} request day off for the dates ${notification_obj.dates_submitted} (Total days: ${notification_obj.total_days}) with reason: <br/> <b>${notification_obj.reason}</b>`;
    }
    else {
      text = `A manager has ${notification_obj.not_status == 1 ? "approved" : "rejected"} your application that requested at ${notification_obj.date_submitted} for day off for the dates ${notification_obj.dates_submitted} (Total days: ${notification_obj.total_days}) with reason: <br/> <b>${notification_obj.reason}</b>`;
    }
    ...
    if (notification_obj.notification_action != 0 || notification_obj.statusId != 0) {
      ...
      $('.notification-actions').attr('data-uuid', `${notification_obj.application_guid}`);
    }

    readNotification( parseInt($(this).attr('data-id')) );

  });

  $(document).on('click', '.notification-actions', function () {
    var action = parseInt($(this).attr('data-action'));
    var uuid = $(this).attr('data-uuid');

    $.ajax({
      url: `/api/${action == 1 ? 'application-approve' : 'application-reject'}/${uuid}`,
      type: 'get',
      contentType: 'application/json; charset=utf-8',
      success: function (response) {
        if (response.status) {
          Swal.fire(
            'Success',
            `${action == 1 ? 'You have approve the appliation.' : 'You have reject the application.'}`,
            'success'
          ).then(function (dialogue_result) {
            if (dialogue_result.isConfirmed) {
              document.location.reload();
  ...
  function readNotification(notification_id) {
    $.ajax({
      url: `/api/notification/readNotification/${notification_id}`,
      type: 'get',
      async: false,
      contentType: 'application/json; charset=utf-8',
      success: function (response) {
